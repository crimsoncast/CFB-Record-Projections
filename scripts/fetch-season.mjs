...
/* A conference championship game is a same-conference matchup between two FBS
   teams whose notes identify it as a championship. Detection is deliberately
   notes-only. An earlier version also treated any same-conference game in the
   final week as a title game, which caught the Army-Navy Game every season:
   both academies are American Athletic members and their rivalry is played the
   week after championship weekend, so it owns the last week of the calendar.
   Bowl and playoff games are excluded explicitly, since those pair
   conference-mates too. */
function isChampionship(g, fbs) {
...
async function fetchSeason(year) {
  ...
  const championships = [...allRegular, ...allPost]
    .filter((g) => isChampionship(g, fbs))
    .filter((g) => {
      if (seenConf.has(g.homeConf)) return false;
